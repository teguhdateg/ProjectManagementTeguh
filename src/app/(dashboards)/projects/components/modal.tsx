...
export default function Modal({ type, id }: ModalProps) {
  ...
  console.log(type);
  const { data: projectDetail } = useProjectsGetById({ id: id ?? "" }) as {
  ...
  useEffect(() => {
    if (type === "edit" && projectDetail) {
      ...
      form.setValue(
        "teamEmails",
        projectDetail.data.teams.map((team) => team.email)
      );
  ...
  const { mutate: addProject } = useProjectPost({
    onSuccess: (res) => {
      console.log(res);
    },
    onError: (err) => {
  ...
  return (
    <>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {type === "add" ? "Add New Project" : "Edit Project"}
          </DialogTitle>
          <DialogDescription>
